...
// │ █████████████████ CTAS USIM HEADER ███████████████████████ │
// ├─────────────────────────────────────────────────────────────┤
// │ 🔖 hash_id      : USIM-DATABASE-FACTORY-0001               │
// │ 📁 domain       : Database, Integration                     │
// │ 🧠 description  : Database connector factory for creating   │
// │                  and managing database connections          │
// │ 🕸️ hash_type    : UUID → CUID-linked module                 │
// │ 🔄 parent_node  : NODE_INTEGRATION                         │
// │ 🧩 dependencies : config, supabase, neo4j, mongodb         │
...
---------------------
Factory module for creating and managing database connections
to Supabase, Neo4j, and MongoDB based on configuration.
"""
...
logger = logging.getLogger("database_factory")
...
class DatabaseFactory:
    ...
    def get_connector(self, db_type: DatabaseType) -> Any:
        """
        Get or create a connector for the specified database type
        ...
        # Create new connector based on database type
        if db_type == DatabaseType.SUPABASE:
            self.connectors[db_type] = SupabaseConnector(
        ...
        logger.info(f"Created connector for {db_type}")
        return self.connectors[db_type]

    def get_supabase(self) -> "SupabaseConnector":
        """
        ...
        Returns:
            Supabase connector instance
        """
        return self.get_connector(DatabaseType.SUPABASE)

    def get_neo4j(self) -> "Neo4jConnector":
        """
        Get Neo4j connector
    ...
    def get_mongodb(self) -> "MongoDBConnector":
        """
