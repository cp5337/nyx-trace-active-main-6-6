// =====================================================
// CTAS ENHANCED MONOREPO SCANNER v6.6 - FIXED VERSION
// =====================================================
...
impl CTASEnhancedScanner {
    pub fn new() -> Self {
        ...
        compiled_regexes.insert("hashid".to_string(), Regex::new(r"hashid:\s*([a-f0-9]{8,64})").unwrap());
        compiled_regexes.insert("cuid".to_string(), Regex::new(r"cuid:\s*([a-z0-9]{20,32})").unwrap());
        compiled_regexes.insert("ttl".to_string(), Regex::new(r"ttl:\s*(\d+)").unwrap());
        compiled_regexes.insert("rdf_triplet".to_string(), Regex::new(r"RDF:\s*(\S+)\s+(\S+)\s+(\S+)").unwrap());
        
        // Fixed secret detection patterns
        compiled_regexes.insert("password".to_string(), Regex::new(r"password\s*=\s*['\"][^'\"]+['\"]").unwrap());
        compiled_regexes.insert("api_key".to_string(), Regex::new(r"api_key\s*=\s*['\"][^'\"]+['\"]").unwrap());
        compiled_regexes.insert("secret".to_string(), Regex::new(r"secret\s*=\s*['\"][^'\"]+['\"]").unwrap());
        compiled_regexes.insert("token".to_string(), Regex::new(r"token\s*=\s*['\"][a-zA-Z0-9]{20,}['\"]").unwrap());
        ...
        if ignore_path.exists() {
            match fs::read_to_string(ignore_path) {
                Ok(content) => content
                    ...
                    .collect(),
                Err(e) => {
                    eprintln!("Warning: Could not read .ctas-ignore: {}", e);
                    vec![]
                }
            }
        } else {
    ...
    fn validate_usim_compliance(&self, contents: &str) -> USIMCompliance {
        let hashid_regex = &self.compiled_regexes["hashid"];
        let cuid_regex = &self.compiled_regexes["cuid"];
        let ttl_regex = &self.compiled_regexes["ttl"];
    ...
    fn validate_rdf_compliance(&self, contents: &str) -> RDFCompliance {
        ...
        let rdf_regex = &self.compiled_regexes["rdf_triplet"];
    ...
    /// Analyze security compliance with improved error handling
    fn analyze_security_compliance(&self, contents: &str) -> SecurityCompliance {
        ...
        // Check for hardcoded secrets using pre-compiled regex patterns
        let secret_patterns = ["password", "api_key", "secret", "token"];
        let no_hardcoded_secrets = !secret_patterns.iter()
            .any(|pattern_name| {
                if let Some(regex) = self.compiled_regexes.get(*pattern_name) {
    ...
    /// Execute Dr. Langford's validation on a single file with improved error handling
    ...
    pub fn scan_repository(&self, repo_path: &Path) -> CTASValidationReport {
        ...
        if let Err(e) = self.visit_dirs_enhanced(repo_path, &mut |entry: &DirEntry| {
            let path = entry.path();
